...
const getOrderById = async (req, res) => {
  try {
    const order = await Order.findById(req.params.id)
      .populate('eventId', 'nombre fecha hora lugar imagen artistas precio');
    if (!order) {
    ...
    res.status(500).json({ mensaje: 'Error al obtener los detalles de la orden', detalle: error.message });
  }
};
...
const createOrder = async (req, res) => {
  try {
    ...
    } else if (lowerMetodo.includes('qr_mercadopago') || lowerMetodo.includes('mercadopago_qr') || lowerMetodo.includes('pago por qr mp')) {
      normalizedMetodoPago = 'qr_mercadopago';
